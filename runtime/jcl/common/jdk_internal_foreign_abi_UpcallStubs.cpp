...
/**
 * A memory segment is associated with a native scope owned by a thread, in which case
 * the memory segment belonging to the scope will be automatically released in OpenJDK
 * by invoking this function when the native scope is terminated.
 ...
 * to be allocated on the heap for thunk.
 */
jboolean JNICALL
Java_jdk_internal_foreign_abi_UpcallStubs_freeUpcallStub0(JNIEnv *env, jobject receiver, jlong address)
{
...
#if defined(AIXPPC) || (defined(S390) && defined(LINUX))
...
#else /* defined(AIXPPC) || (defined(S390) && defined(LINUX)) */
	void *thunkAddr = (void *)(UDATA)address;
#endif /* defined(AIXPPC) || (defined(S390) && defined(LINUX)) */
	...
	if (NULL != thunkAddr) {
		...
		if (NULL != metaDataHashTable) {
			...
			metaDataEntry.thunkAddrValue = (UDATA)(intptr_t)thunkAddr;
			...
			if (NULL != result) {
				...
				if (NULL != metaData) {
					...
					vmFuncs->j9jni_deleteGlobalRef(env, metaData->calleeMH, JNI_FALSE);
					printf("\nfreeUpcallStub0: release the upcall metadata: thunkAddrValue = 0x%lx, mhMetaData = 0x%p\n", metaDataEntry.thunkAddrValue, metaData->mhMetaData);
				...
				if (NULL != thunkHeap) {
					printf("\nfreeUpcallStub0: release the upcall funcAddr = 0x%llx, thunkAddr = %p\n", address, thunkAddr);
