...
Java_java_lang_Thread_getThreads(JNIEnv *env, jclass clazz)
{
	...
	if (NULL == threads) {
		...
		if (NULL != arrayClass) {
			j9object_t arrayObject = vm->memoryManagerFunctions->J9AllocateIndexableObject(currentThread, arrayClass, threadCount, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
			if (NULL != arrayObject) {
