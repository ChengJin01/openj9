			...
			if (0 == ffiArgs[i]) {
				...
				if (J9_FFI_DOWNCALL_HEAP_ARGUMENT_ID == pointerValues[i]) {
					...
					if (J9ISCONTIGUOUSARRAY(_currentThread, heapBase)) {
						...
						void *elems = memcpyFromObjectArray(_currentThread, heapBase, &isCopy, JNI_FALSE);
		...
		if (isHeapPassed) {
			...
			for (U_8 i = 0; i < ffiArgCount; i++) {
				...
				if ((J9NtcPointer == argType) && (J9_FFI_DOWNCALL_HEAP_ARGUMENT_ID == (U_64)ffiArgs[i])) {
					...
					if (!J9ISCONTIGUOUSARRAY(_currentThread, heapBase)) {
						memcpyToObjectArray(_currentThread, heapBase, (void *)(UDATA)(pointerValues[i]), 0, JNI_FALSE);
					}
					curHeapArgIdx += 1;
				}
			}
