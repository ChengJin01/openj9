...
throwStackOverflow:
	...
	{
		VM_BytecodeAction rc = GOTO_RUN_METHOD;
		j9object_t mhMetaData = (j9object_t)_currentThread->returnValue2;
retry:
		...
		if (J9_EXPECTED(NULL != invokeCacheArray)) {
			 ...
			 * a MemberName object plus appendix which are stored in a Java Object array
			 * defined in UpcallMHMetaData.
			 * see MethodHandleResolver.linkCallerMethod() for details.
			 */
			printf("\n native2InterpreterTransition: calling resolveUpcallInvokeHandle -----\n");
