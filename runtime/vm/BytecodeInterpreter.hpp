...
nativeOOM:
	...
	inlProgrammableInvokerInvokeNative(REGISTER_ARGS_LIST)
	{
		...
		if (J9NtcStruct == returnType) {
			...
			printf("\ninlProgrammableInvokerInvokeNative: ffiRetType->size = %d, ffiRetType->alignment = %d\n",
					(int)ffiRetType->size, (int)ffiRetType->alignment);
#if defined(WIN64)
			...
			printf("\ninlProgrammableInvokerInvokeNative: structValueSize = %d\n", (int)structValueSize);
#endif /* defined(WIN64) */
