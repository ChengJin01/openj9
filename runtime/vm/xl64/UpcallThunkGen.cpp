...
createUpcallThunk(J9UpcallMetaData *metaData)
{
	...
	thunkMem = (I_32 *)vmFuncs->allocateUpcallThunkMemory(vm, metaData, metaData->thunkSize);
	if (NULL == thunkMem) {
		return NULL;
	}
	...
	*(J9UpcallMetaData **)((char *)thunkMem + roundedCodeSize) = metaData;
	...
	//printf("\nJIT: createUpcallThunk:  ---- END ------: thunkMem = %p", thunkMem);
