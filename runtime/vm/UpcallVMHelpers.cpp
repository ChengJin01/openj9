...
static j9object_t createMemAddressObject(J9UpcallMetaData *data, I_64 offset, bool isCurThrdAllocated);
static j9object_t createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize, bool isCurThrdAllocated, j9object_t sessionOrScopeObject);
static j9object_t getSessionOrScopeObject(J9UpcallMetaData *data, bool isCurThrdAllocated);
	...
	if (buildCallInStackFrameHelper(currentThread, &newELS, returnsObject)) {
		...
		for (I_32 argIndex = 0; argIndex < paramCount; argIndex++) {
			...
			case J9_FFI_UPCALL_SIG_TYPE_POINTER:
			{
				...
				j9object_t memAddrObject = createMemAddressObject(data, offset, isCurThrdAllocated);
			...
			case J9_FFI_UPCALL_SIG_TYPE_STRUCT:
			{
				...
				j9object_t memSegmtObject = createMemSegmentObject(data, offset, sigArray[argIndex].sizeInByte, isCurThrdAllocated, sessionOrScopeObject);
 ...
 * @param isCurThrdAllocated a flag indicating whether the current thread is allocated locally
 * @return a MemoryAddress object
 */
static j9object_t
createMemAddressObject(J9UpcallMetaData *data, I_64 offset, bool isCurThrdAllocated)
{
	J9JavaVM * vm = data->vm;
	J9VMThread *currentThread = vm->internalVMFunctions->currentVMThread(vm);
	...
	if (NULL == memAddrObject) {
		...
		if ((NULL == memAddrObject) && (!isCurThrdAllocated)) {
 ...
 */
static j9object_t
createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize, bool isCurThrdAllocated, j9object_t sessionOrScopeObject)
{
	J9JavaVM *vm = data->vm;
	J9VMThread *currentThread = vm->internalVMFunctions->currentVMThread(vm);
	MM_ObjectAllocationAPI objectAllocate(currentThread);
	...
	if (NULL == sessionOrScopeObject) {
		sessionOrScopeObject = getSessionOrScopeObject(data, isCurThrdAllocated);
		if (NULL == sessionOrScopeObject) {
	...
	if (NULL == memSegmtObject) {
		...
		if ((NULL == memSegmtObject) && (!isCurThrdAllocated)) {
			setHeapOutOfMemoryError(currentThread);
			goto done;
		}
	}
 ...
 */
static j9object_t
getSessionOrScopeObject(J9UpcallMetaData *data, bool isCurThrdAllocated)
{
	J9JavaVM *vm = data->vm;
	J9VMThread *currentThread = vm->internalVMFunctions->currentVMThread(vm);
	MM_ObjectAllocationAPI objectAllocate(currentThread);
	...
	if (!isCurThrdAllocated) {
#if JAVA_SPEC_VERSION >= 19
		sessionOrScopeObject = J9VMOPENJ9INTERNALFOREIGNABIUPCALLMHMETADATA_SESSION(currentThread, J9_JNI_UNWRAP_REFERENCE(data->mhMetaData));
#else /* JAVA_SPEC_VERSION >= 19 */
		sessionOrScopeObject = J9VMOPENJ9INTERNALFOREIGNABIUPCALLMHMETADATA_SCOPE(currentThread, J9_JNI_UNWRAP_REFERENCE(data->mhMetaData));
#endif /* JAVA_SPEC_VERSION >= 19 */
	}
	...
	if (NULL == sessionOrScopeObject) {
		...
		if (NULL == sessionOrScopeObject) {
			...
			if ((NULL == sessionOrScopeObject) && (!isCurThrdAllocated)) {
				setHeapOutOfMemoryError(currentThread);
			}
		}
	}
