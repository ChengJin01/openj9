...
static j9object_t createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize, bool isAllocated, j9object_t scopeOrSessionObject);
	...
	if (buildCallInStackFrameHelper(currentThread, &newELS, returnsObject)) {
		...
		for (I_32 argIndex = 0; argIndex < paramCount; argIndex++) {
			...
			case J9_FFI_UPCALL_SIG_TYPE_STRUCT:
			{
				...
				j9object_t memSegmtObject = createMemSegmentObject(data, offset, sigArray[argIndex].sizeInByte, isAllocated, scopeOrSessionObject);
 ...
 * @param isAllocated a flag indicating whether the current thread is allocated locally
 * @param scopeOrSessionObject the session/scope object created for arguments
 * @return a MemorySegment object
 */
static j9object_t
createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize, bool isAllocated, j9object_t scopeOrSessionObject)
{
	J9JavaVM *vm = data->vm;
	J9VMThread *currentThread = vm->internalVMFunctions->currentVMThread(vm);
	MM_ObjectAllocationAPI objectAllocate(currentThread);
