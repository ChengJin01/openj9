...
createMemAddressObject(J9UpcallMetaData *data, I_64 offset)
{
	...
	J9VMJDKINTERNALFOREIGNMEMORYADDRESSIMPL_SET_SEGMENT(currentThread, memoryAddrObject, NULL);
...
/**
 * @brief Get the address to the native memory from a MemoryAddress object.
 *
 * @param data the pointer to J9UpcallMetaData
 * @param memoryAddrObject a MemoryAddress object
 * @return the native pointer to the requested memory
 *
 * Note:
...
static long getNativePtrFromMemAddressObject(J9UpcallMetaData *data, j9object_t memAddrObject)
{
	...
	if (NULL != segmtObject) { // offset is set to zero in AbstractMemorySegmentImpl.address() in OpenJDK
