...
getReturnTypeFromMetaData(J9UpcallMetaData *data)
{
	...
	j9object_t methodType = J9VMJAVALANGINVOKEMETHODHANDLE_TYPE(currentThread,
			J9_JNI_UNWRAP_REFERENCE(data->calleeMH));
...
icallVMprJavaUpcallImpl(J9UpcallMetaData *data, void *argsListPointer)
{
	...
	*returnStorage = 0;
	...
	if (buildCallInStackFrameHelper(currentThread, &newELS)) {
		 ...
		 * on the stack by GC when allocating memory for pointer or struct.
		 */
		PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, J9_JNI_UNWRAP_REFERENCE(data->calleeMH));
