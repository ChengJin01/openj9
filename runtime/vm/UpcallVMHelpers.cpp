...
static void convertUpcallReturnValue(J9VMThread *currentThread, U_8 returnType, UDATA *returnStorage);
...
static j9object_t createResourceScopeObject(J9UpcallMetaData *data);
static UDATA getNativeAddrFromMemAddressObject(J9VMThread *currentThread, j9object_t memAddrObject);
...
{
	J9VMThread *currentThread = data->ownerThread;
	icallVMprJavaUpcallImpl(data, argsListPointer);
...
getReturnTypeFromMetaData(J9UpcallMetaData *data)
{
	...
	j9object_t calleeHandle = J9VMJDKINTERNALFOREIGNABIUPCALLMHMETADATA_CALLEEMH(currentThread,
			J9_JNI_UNWRAP_REFERENCE(data->mhMetaData));
	j9object_t methodType = J9VMJAVALANGINVOKEMETHODHANDLE_TYPE(currentThread, calleeHandle);
...
icallVMprJavaUpcallImpl(J9UpcallMetaData *data, void *argsListPointer)
{
	...
	bool returnsObject = ((J9NtcPointer == returnType) || (J9NtcStruct == returnType)) ? true : false;
	J9UpcallNativeSignature *nativeSig = data->nativeFuncSignature;
	J9UpcallSigType *sigArray = nativeSig->sigArray;
	...
	*returnStorage = 0;
	...
	if (buildCallInStackFrameHelper(currentThread, &newELS, returnsObject)) {
		...
		/* Save the current literals for later restoration */
		...
		for (UDATA argIndex = 0; argIndex < paramCount; argIndex++) {
			...
			case J9_FFI_UPCALL_SIG_TYPE_POINTER:
			{
				...
				if (NULL == memAddrObject) {
					 ...
					 * from the downcall to the native function.
					 */
					goto done;
				}
		 ...
		 * are passed over to native2InterpreterTransition() in the interpreter to invoke
	...
	printf("\nicallVMprJavaUpcallImpl_0: *returnStorage = 0x%lx, returnTypeByteSize = %d", *returnStorage, sigArray[paramCount].sizeInByte);
	convertUpcallReturnValue(currentThread, returnType, returnStorage);
 ...
 *
 * @param currentThread The pointer to J9VMThread
 * @param returnType[in] The type of the return value
 * @param returnStorage[in] The pointer to the return value
 */
static void
convertUpcallReturnValue(J9VMThread *currentThread, U_8 returnType, UDATA *returnStorage)
{
	switch (returnType) {
	...
	case J9NtcPointer:
	{
		...
		*returnStorage = getNativeAddrFromMemAddressObject(currentThread, memAddrObject);
		break;
	}
	case J9NtcStruct:
	{
		j9object_t memSegmtObject = (j9object_t)*returnStorage;
		/* Get the native address to the requested struct from a MemorySegment object */
		*returnStorage = (UDATA)J9VMJDKINTERNALFOREIGNNATIVEMEMORYSEGMENTIMPL_MIN(currentThread, memSegmtObject);
		Assert_VM_true(0 != *returnStorage);
		break;
	}
	default:
		break;
...
createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize)
{
	...
	scopeObject = createResourceScopeObject(data);
	if (NULL == scopeObject) {
 ...
 */
static j9object_t
createResourceScopeObject(J9UpcallMetaData *data)
{
	J9VMThread *currentThread = data->ownerThread;
	J9JavaVM *vm = currentThread->javaVM;
	MM_ObjectAllocationAPI objectAllocate(currentThread);
 ...
 */
static UDATA
getNativeAddrFromMemAddressObject(J9VMThread *currentThread, j9object_t memAddrObject)
{
	UDATA offset = (UDATA)J9VMJDKINTERNALFOREIGNMEMORYADDRESSIMPL_OFFSET(currentThread, memAddrObject);
	UDATA nativePtrValue = offset;
	...
	if (NULL != segmtObject) {
		nativePtrValue = (UDATA)J9VMJDKINTERNALFOREIGNNATIVEMEMORYSEGMENTIMPL_MIN(currentThread, segmtObject);
	}
#endif /* JAVA_SPEC_VERSION <= 17 */
