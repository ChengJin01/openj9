...
createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize, bool isAllocated)
{
	...
	j9object_t session = J9VMOPENJ9INTERNALFOREIGNABIUPCALLMHMETADATA_SESSION(currentThread, J9_JNI_UNWRAP_REFERENCE(data->mhMetaData));
#else /* JAVA_SPEC_VERSION >= 19 */
	j9object_t scope = J9VMOPENJ9INTERNALFOREIGNABIUPCALLMHMETADATA_SCOPE(currentThread, J9_JNI_UNWRAP_REFERENCE(data->mhMetaData));
...
#if JAVA_SPEC_VERSION >= 19
	J9VMJDKINTERNALFOREIGNNATIVEMEMORYSEGMENTIMPL_SET_SESSION(currentThread, memSegmtObject, session);
#else /* JAVA_SPEC_VERSION >= 19 */
	J9VMJDKINTERNALFOREIGNNATIVEMEMORYSEGMENTIMPL_SET_SCOPE(currentThread, memSegmtObject, scope);
#endif /* JAVA_SPEC_VERSION >= 19 */
