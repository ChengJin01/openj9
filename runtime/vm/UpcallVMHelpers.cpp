...
#define FFI_UPCALL_J9CLASS_EYECATCHER (UDATA)0x99669966
...
icallVMprJavaUpcallImpl(J9UpcallMetaData *data, void *argsListPointer)
{
	...
	*returnStorage = 0;
	...
	if (buildCallInStackFrameHelper(currentThread, &newELS)) {
		...
		/* The argument list of the upcall method handle on the java stack includes the target method handle,
		...
		*(j9object_t*)--currentThread->sp = calleeHandle;
		...
		for (UDATA argIndex = 0; argIndex < lastParamIdx; argIndex++) {
			...
			case J9_FFI_UPCALL_SIG_TYPE_POINTER:
			{
				...
				Assert_VM_true(NULL != memAddrObject);
			...
			case J9_FFI_UPCALL_SIG_TYPE_STRUCT:
			{
				...
				Assert_VM_true(NULL != memSegmtObject);
...
createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize)
{
	...
	Assert_VM_true(NULL != scopeObject);
