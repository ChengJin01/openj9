...
static j9object_t createResourceScopeObject(J9UpcallMetaData *data, j9object_t memSegmtObject);
...
getReturnTypeFromMetaData(J9UpcallMetaData *data)
{
	...
	j9object_t methodType = J9VMJDKINTERNALFOREIGNABIUPCALLMHMETADATA_CALLEETYPE(currentThread, mhMetaData);
...
icallVMprJavaUpcallImpl(J9UpcallMetaData *data, void *argsListPointer)
{
	...
	*returnStorage = 0;
	...
	if (buildCallInStackFrameHelper(currentThread, &newELS)) {
		 ...
		 */
		currentThread->sp -= 1;
		data->spForCalleeMH = currentThread->sp;
		...
		/* Pass mhMetaData as argument to native2InterpreterTransition() when calling into
...
createMemSegmentObject(J9UpcallMetaData *data, I_64 offset, U_32 sigTypeSize)
{
	...
	scopeObject = createResourceScopeObject(data, memSegmtObject);
	if (J9_UNEXPECTED(NULL == scopeObject)) {
		/* The OOM exception is already set in createResourceScopeObject() and will be thrown
		 * in inlProgrammableInvokerInvokeNative() when return from the interpreter, in which
		 * case there is no need to release the allocated MemorySegment object.
		 */
		memSegmtObject = NULL;
		goto done;
	}
 ...
 */
static j9object_t
createResourceScopeObject(J9UpcallMetaData *data, j9object_t memSegmtObject)
{
	J9JavaVM *vm = data->vm;
	J9VMThread *currentThread = vm->internalVMFunctions->currentVMThread(vm);
	j9object_t mhMetaData = NULL;
	...
	if (NULL == scopeObject) {
		VM_VMHelpers::pushObjectInSpecialFrame(currentThread, memSegmtObject);
		scopeObject = vm->memoryManagerFunctions->J9AllocateObject(currentThread, scopeClass, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
		memSegmtObject = VM_VMHelpers::popObjectInSpecialFrame(currentThread);
		if (J9_UNEXPECTED(NULL == scopeObject)) {
			setHeapOutOfMemoryError(currentThread);
			goto done;
		}
