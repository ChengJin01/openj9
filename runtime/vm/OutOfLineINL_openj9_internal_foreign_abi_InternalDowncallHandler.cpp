...
OutOfLineINL_openj9_internal_foreign_abi_InternalDowncallHandler_resolveRequiredFields(J9VMThread *currentThread, J9Method *method)
{
	...
	static const U_16 cpIndex[] = {
			...
			,J9VMCONSTANTPOOL_OPENJ9INTERNALFOREIGNABIINTERNALDOWNCALLHANDLER_NATIVEMH
...
OutOfLineINL_openj9_internal_foreign_abi_InternalDowncallHandler_isFfiProtoEnabled(J9VMThread *currentThread, J9Method *method)
{
	VM_BytecodeAction rc = EXECUTE_BYTECODE;
	J9JavaVM *vm = currentThread->javaVM;
	bool isFfiProtoOn = (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags2, J9_EXTENDED_RUNTIME2_FFI_PROTO)) ? true : false;
...
/**
 * openj9.internal.foreign.abi.InternalDowncallHandler: private native void resolveBoundHandle();
 *
 ...
 */
VM_BytecodeAction
OutOfLineINL_openj9_internal_foreign_abi_InternalDowncallHandler_resolveBoundHandle(J9VMThread *currentThread, J9Method *method)
{
	VM_BytecodeAction rc = EXECUTE_BYTECODE;
	j9object_t invokeCache = NULL;

	jobject nativeInvoker = j9jni_createGlobalRef((JNIEnv*)currentThread, J9_JNI_UNWRAP_REFERENCE(currentThread->sp), JNI_FALSE);
	if (NULL == nativeInvoker) {
		rc = GOTO_THROW_CURRENT_EXCEPTION;
		setNativeOutOfMemoryError(currentThread, 0, 0);
		goto done;
	}
	 ...
	 */
	VM_OutOfLineINL_Helpers::buildInternalNativeStackFrame(currentThread, method);
	invokeCache = resolveFfiCallInvokeHandle(currentThread,
			J9VMOPENJ9INTERNALFOREIGNABIINTERNALDOWNCALLHANDLER_BOUNDMH(currentThread,
					J9_JNI_UNWRAP_REFERENCE(nativeInvoker)));
	if (VM_VMHelpers::exceptionPending(currentThread)) {
		rc = GOTO_THROW_CURRENT_EXCEPTION;
		goto done;
	}
	VM_AtomicSupport::writeBarrier();
	J9VMJAVALANGINVOKENATIVEMETHODHANDLE_SET_INVOKECACHE(currentThread,
			J9VMOPENJ9INTERNALFOREIGNABIINTERNALDOWNCALLHANDLER_NATIVEMH(currentThread,
					J9_JNI_UNWRAP_REFERENCE(nativeInvoker)), invokeCache);
	VM_OutOfLineINL_Helpers::restoreInternalNativeStackFrame(currentThread);

done:
	j9jni_deleteGlobalRef((JNIEnv *)currentThread, nativeInvoker, JNI_FALSE);
