...
/* jdk.internal.foreign.abi.ProgrammableInvoker: private static synchronized native void initInstanceFieldOffset(); */
VM_BytecodeAction
OutOfLineINL_jdk_internal_foreign_abi_ProgrammableInvoker_initInstanceFieldOffset(J9VMThread *currentThread, J9Method *method)
{
	VM_BytecodeAction rc = EXECUTE_BYTECODE;
	J9JavaVM *vm = currentThread->javaVM;
	J9ConstantPool *jclConstantPool = (J9ConstantPool *)vm->jclConstantPool;
	const int cpEntryNum = 3;
	U_16 cpIndex[cpEntryNum] = {
				...
				J9VMCONSTANTPOOL_JDKINTERNALFOREIGNABIPROGRAMMABLEINVOKER_ARGCOUNT
			};

	printf("\n**********calling OutOfLineINL_initInstanceFieldOffset***BEGIN*******\n");
	...
	for (int i = 0; i < cpEntryNum; i++) {
		...
		if (!VM_VMHelpers::instanceFieldRefIsResolved(flags, valueOffset)) {
			UDATA tempValueOffset = cpFieldRef->valueOffset;
			...
			tempValueOffset = cpFieldRef->valueOffset;
			printf("\nOutOfLineINL_initInstanceFieldOffset: exit from resolveInstanceFieldRef: cpIndex[%d] = %d, valueOffset = %d\n", i, (int)cpIndex[i], (int)tempValueOffset);
		}
	}

done:
	printf("\n**********calling OutOfLineINL_initInstanceFieldOffset***END*******\n");
	VM_OutOfLineINL_Helpers::returnVoid(currentThread, 0);
	return rc;
}

/* jdk.internal.foreign.abi.ProgrammableInvoker: private native void initCifNativeThunkData(Class<?>[] argLayoutClasses, Class<?> retLayoutClass, boolean newArgTypes, boolean varArgsExist); */
VM_BytecodeAction
OutOfLineINL_jdk_internal_foreign_abi_ProgrammableInvoker_initCifNativeThunkData(J9VMThread *currentThread, J9Method *method)
{
	...
	bool varArgsExist = (bool)(*(U_32*)currentThread->sp);
	bool newArgTypes = (bool)(*(U_32*)currentThread->sp + 1);
	j9object_t retLayoutClassesObject = J9_JNI_UNWRAP_REFERENCE(currentThread->sp + 2);
	j9object_t argLayoutClassesObject = J9_JNI_UNWRAP_REFERENCE(currentThread->sp + 3);
	...
	if (varArgsExist) {
		/* The number of fixed arguments prior to the varargs must be greater than zero */
		Assert_VM_true(argTypesCount > 1);
		if (FFI_OK != ffi_prep_cif_var(cif, FFI_DEFAULT_ABI, argTypesCount - 1, argTypesCount, returnType, &(argTypes[0]))) {
			rc = GOTO_THROW_CURRENT_EXCEPTION;
			setCurrentException(currentThread, J9VMCONSTANTPOOL_JAVALANGINTERNALERROR, NULL);
			printf("\n**********calling OutOfLineINL_initCifNativeThunkData: ffi_prep_cif_var: NOK!!!: GOTO_THROW_CURRENT_EXCEPTION\n");
			goto freeAllMemoryThenExit;
		}
	} else {
