...
/* jdk.internal.foreign.abi.ProgrammableInvoker: private static synchronized native void resolveRequiredFieldsAndClasses(); */
VM_BytecodeAction
OutOfLineINL_jdk_internal_foreign_abi_ProgrammableInvoker_resolveRequiredFieldsAndClasses(J9VMThread *currentThread, J9Method *method)
{
	VM_BytecodeAction rc = EXECUTE_BYTECODE;
	J9JavaVM *vm = currentThread->javaVM;
	J9Class * memAddrClazz = NULL;
	...
	printf("\n**********calling OutOfLineINL_resolveRequiredFieldsAndClasses***BEGIN*******\n");
	...
	for (int i = 0; i < cpEntryNum; i++) {
		...
		if (!VM_VMHelpers::instanceFieldRefIsResolved(flags, valueOffset)) {
			...
			printf("\nOutOfLineINL_resolveRequiredFieldsAndClasses: exit from resolveInstanceFieldRef: cpIndex[%d] = %d, valueOffset = %d", i, (int)cpIndex[i], (int)cpFieldRef->valueOffset);
		}
	}

	/* The J9Class value of MemoryAddress is used to identify the corresponding ffi_type in getFFIType() */
	memAddrClazz = resolveClassRef(currentThread, jclConstantPool, J9VMCONSTANTPOOL_JDKINCUBATORFOREIGNMEMORYADDRESS, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
	if ((NULL == memAddrClazz) || (NULL != currentThread->currentException)) {
		rc = GOTO_THROW_CURRENT_EXCEPTION;
		goto done;
	}
	omrthread_monitor_enter(vm->exclusiveAccessMutex);
	vm->memoryAddressClass = memAddrClazz;
	omrthread_monitor_exit(vm->exclusiveAccessMutex);

done:
	printf("\n**********calling OutOfLineINL_resolveRequiredFieldsAndClasses***END*******\n");
	VM_OutOfLineINL_Helpers::returnVoid(currentThread, 0);
	return rc;
}

/* jdk.internal.foreign.abi.ProgrammableInvoker: private native void initCifNativeThunkData(Class<?>[] argLayoutClasses, Class<?> retLayoutClass, boolean newArgTypes); */
VM_BytecodeAction
OutOfLineINL_jdk_internal_foreign_abi_ProgrammableInvoker_initCifNativeThunkData(J9VMThread *currentThread, J9Method *method)
{
	...
	j9object_t retLayoutClassesObject = J9_JNI_UNWRAP_REFERENCE(currentThread->sp + 1);
	j9object_t argLayoutClassesObject = J9_JNI_UNWRAP_REFERENCE(currentThread->sp + 2);
