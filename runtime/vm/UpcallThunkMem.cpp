...
allocateUpcallThunkMemory(J9JavaVM *vm, J9UpcallMetaData *data, UDATA thunkSize)
{
	...
	if ((NULL == thunkAddress) && (NULL == vm->thunkHeapWrapper)) {
		printf("\n subAllocateThunkFromHeap:FAILED: thunkAddress = %p, thunkSize = %ld", thunkAddress, thunkSize);
		thunkAddress = allocateThunkHeap(vm, data, thunkSize);
		printf("\n allocateThunkHeap:OK: thunkAddress = %p, thunkSize = %ld", thunkAddress, thunkSize);
	} else {
		printf("\n subAllocateThunkFromHeap:OK: thunkAddress = %p, thunkSize = %ld", thunkAddress, thunkSize);
	}
