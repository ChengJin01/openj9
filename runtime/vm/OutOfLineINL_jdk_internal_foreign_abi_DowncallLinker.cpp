...
/* jdk.internal.foreign.abi.DowncallLinker: private static synchronized native void resolveRequiredFields(); */
VM_BytecodeAction
OutOfLineINL_jdk_internal_foreign_abi_DowncallLinker_resolveRequiredFields(J9VMThread *currentThread, J9Method *method)
{
	...
	U_16 cpIndex[cpEntryNum] = {
				J9VMCONSTANTPOOL_JDKINTERNALFOREIGNABIDOWNCALLLINKER_CIFNATIVETHUNKADDR,
				J9VMCONSTANTPOOL_JDKINTERNALFOREIGNABIDOWNCALLLINKER_ARGTYPESADDR
			};
...
/**
 * jdk.internal.foreign.abi.DowncallLinker: private native void initCifNativeThunkData(String[] argLayouts, String retLayout, boolean newArgTypes);
 *
 ...
 */
VM_BytecodeAction
OutOfLineINL_jdk_internal_foreign_abi_DowncallLinker_initCifNativeThunkData(J9VMThread *currentThread, J9Method *method)
{
	VM_BytecodeAction rc = EXECUTE_BYTECODE;
	J9JavaVM *vm = currentThread->javaVM;
	...
	if (!newArgTypes) {
		argTypes = (ffi_type **)(UDATA)J9VMJDKINTERNALFOREIGNABIDOWNCALLLINKER_ARGTYPESADDR(currentThread, nativeInvoker);
	} else {
	...
	if (newArgTypes) {
		...
		J9VMJDKINTERNALFOREIGNABIDOWNCALLLINKER_SET_ARGTYPESADDR(currentThread, nativeInvoker, (intptr_t)argTypes);
	}

	VM_AtomicSupport::writeBarrier();
	J9VMJDKINTERNALFOREIGNABIDOWNCALLLINKER_SET_CIFNATIVETHUNKADDR(currentThread, nativeInvoker, (intptr_t)cif);
