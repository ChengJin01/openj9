...
freeUpcallMetaDataDoFn(J9UpcallMetaDataEntry *entry, void *userData)
{
	...
	if ((NULL != thunkAddr) && (NULL != metaData)) {
		...
		vmFuncs->j9jni_deleteGlobalRef((JNIEnv *)currentThread, metaData->calleeMH, JNI_FALSE);
		...
		printf("\n freeUpcallMetaDataDoFn: release the upcall metadata: thunkAddr = 0x%p, mhMetaData = 0x%p\n", thunkAddr, metaData->mhMetaData);
	}
	return JNI_OK;
}
#endif /* JAVA_SPEC_VERSION >= 16 */
