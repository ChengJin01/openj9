...
LayoutFFITypeHelpers::freeStructFFIType(ffi_type *ffiType)
{
	if ((NULL != ffiType) && (FFI_TYPE_STRUCT == ffiType->type)) {
		if (NULL != ffiType->elements) {
			...
			ffiType = NULL;
			/* TODO: we need a better way to deal with inlined arrays,
			 * the current solution just makes a struct with identical types.
			 * We need to investigate whether libffi has support for arrays.
			 */
		}
	}
}
