...
LayoutFFITypeHelpers::getStructFFITypeElements(char **layout)
{
	...
	while ('\0' != *currentLayout) {
		...
		case 'x': /* Create padding elements if exist in the struct */
		{
			UDATA paddingBits = 0;
			currentLayout += 1; // Skip over 'x' to the padding bits
			paddingBits = getIntFromLayout(&currentLayout);
			if (24 == paddingBits) { // 3 bytes (C_CHAR)
				UDATA paddingCount = paddingBits / 8;
				while (paddingCount > 0) {
					elements[nElements] = &ffi_type_sint8;
					nElements += 1;
					paddingCount -= 1;
				}
			} else { // 1 byte (C_CHAR), 2 bytes (C_SHORT) or 4 bytes (C_INT)
				elements[nElements] = getPaddingFFIType(paddingBits);
				nElements += 1;
			}
			break;
		}
