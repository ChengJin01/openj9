...
LayoutFFITypeHelpers::getStructFFIType(char **layout)
{
	...
	if (NULL == _vm->layoutStrFFITypeTable) {
		_vm->layoutStrFFITypeTable = createLayouStrFFITypeTable(_vm);
		if (NULL == _vm->layoutStrFFITypeTable) {
	...
	/* Search the hashtable for the ffi_type if the same strcut layout string exists. */
	currentLayout += 1; /* Skip over the '#' case for the length of the struct layout string. */
	layoutStrFFITypeEntry->layoutStrLength = getIntFromLayout(&currentLayout);
	...
	memcpy(layoutStrFFITypeEntry->layoutStr, *layout, layoutStrFFITypeEntry->layoutStrLength);
	layoutStrFFITypeEntry->layoutStr[layoutStrFFITypeEntry->layoutStrLength] = '\0';
	resultEntry = findLayouStrFFIType(_vm->layoutStrFFITypeTable, layoutStrFFITypeEntry);
	if (NULL != resultEntry) {
	 ...
	 * if the corresponding layout string doesn't exists.
	 */
	layoutStrFFITypeEntry->structFFIType = (void *)typeFFI;
	resultEntry = addLayouStrFFIType(_vm->layoutStrFFITypeTable, layoutStrFFITypeEntry);
	if (NULL == resultEntry) {
