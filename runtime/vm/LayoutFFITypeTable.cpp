...
layoutStrFFITypeHashFn(void *entry, void *userData)
{
	...
	return layoutStrEntry->layoutStrHash;
}

/**
 * @brief Compare the hash values of the two layout strings for equality.
 *
...
layoutStrFFITypeHashEqualFn(void *leftEntry, void *rightEntry, void *userData)
{
	...
	return (leftLayoutStrEntry->layoutStrHash == rightLayoutStrEntry->layoutStrHash);
}
...
createLayoutStrFFITypeTable(J9JavaVM *vm)
{
	return hashTableNew(OMRPORT_FROM_J9PORT(vm->portLibrary), "Layout string table", 0,
			sizeof(J9LayoutStrFFITypeEntry), 0, 0, J9MEM_CATEGORY_VM_FFI, layoutStrFFITypeHashFn, layoutStrFFITypeHashEqualFn, NULL, NULL);
}
