...
OutOfLineINL_jdk_internal_foreign_abi_ProgrammableUpcallHandler_allocateUpcallStub(J9VMThread *currentThread, J9Method *method)
{
	...
	/* Set the fields of J9UpcallMetaData struct required for the thunk generation
	 * and icallVMprJavaUpcallImpl in the upall.
	 *
	 * Note: the current thread must be reserved in the struct given currentVMThread(vm)
	 * returns zero in async upcall tests.
	 */
	upcallMetaData->ownerThread = currentThread;
	...
	resolveUpcallInvokeHandle(upcallMetaData);
	...
	/* Generate the thunk intended for the upcall */
