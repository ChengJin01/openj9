...
public:
	...
	/**
	 * @brief obtain the ffi_type of the padding bits
	 * @param paddingBits[in] The specified padding bits
	 * @return The pointer to the ffi_type corresponding to the padding bits
	 */
	VMINLINE ffi_type*
	getPaddingFFIType(UDATA paddingBits)
	{
		ffi_type* typeFFI = NULL;
		UDATA numOfBytes = paddingBits / 8;
	...
	getLayoutFFIType(ffi_type** typeFFI, j9object_t layoutStringObject)
	{
		...
		*typeFFI = NULL;
		...
		char *layout = copyStringToUTF8WithMemAlloc(_currentThread, layoutStringObject,
													...
													layoutBuffer, sizeof(layoutBuffer), NULL);
		...
		/* Check the layout's size that is prefixed with layout.byteSize() */
	 ...
	 *
	 * Note:
	 * 1) For better parsing the layout string, all the descriptions in the layout string are removed with
	 *    preprocessLayoutString() in ProgrammableInvoker ahead of time.
