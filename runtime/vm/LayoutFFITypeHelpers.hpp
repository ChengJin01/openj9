...
public:
	 ...
	 * e.g. if a struct layout string is "#2[II]", it returns 6 in length.
	 *
	 * @return The length of the struct layout string
	 */
	static VMINLINE UDATA
	getLengthOfStructLayout(char *layout)
	{
		...
		while ('\0' != *layout) {
			...
			case '[':
				...
				break;
			case ']':
				/* Never reach here as a struct layout string always comes from '[' and ']' in pair. */
				if (0 == bracketCounter) {
					Assert_VM_unreachable();
				}
				bracketCounter -= 1;
				/* Jump out of the loop as we reach the boundary of the specified struct layout string. */
				if (0 == bracketCounter) {
					length += 1;
					goto matchAndExit;
				}
...
matchAndExit:
	 ...
	 *
	 * @param currentStructSig[in] A pointer to the current location of a struct signature string
