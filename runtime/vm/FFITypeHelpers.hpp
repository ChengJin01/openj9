...
public:
	...
	getFFIType(J9Class *type){
		...
		omrthread_monitor_enter(_vm->exclusiveAccessMutex);
		if (NULL == _vm->memoryAddressClass) {
			J9InternalVMFunctions const * const vmFuncs = _vm->internalVMFunctions;
			J9ConstantPool *jclConstantPool = (J9ConstantPool *)_vm->jclConstantPool;
			J9ROMStringRef *romStringRef = &((J9ROMStringRef *)jclConstantPool->romConstantPool)[J9VMCONSTANTPOOL_JDKINCUBATORFOREIGNMEMORYADDRESS];
			J9UTF8 *utfWrapper = J9ROMSTRINGREF_UTF8DATA(romStringRef);
			_vm->memoryAddressClass = vmFuncs->internalFindClassUTF8(_currentThread, J9UTF8_DATA(utfWrapper), J9UTF8_LENGTH(utfWrapper), _vm->systemClassLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
		}
		if (NULL == _vm->cLinkerVaListClass) {
			J9InternalVMFunctions const * const vmFuncs = _vm->internalVMFunctions;
			J9ConstantPool *jclConstantPool = (J9ConstantPool *)_vm->jclConstantPool;
			J9ROMStringRef *romStringRef = &((J9ROMStringRef *)jclConstantPool->romConstantPool)[J9VMCONSTANTPOOL_JDKINCUBATORFOREIGNCLINKERVALIST];
			J9UTF8 *utfWrapper = J9ROMSTRINGREF_UTF8DATA(romStringRef);
			_vm->cLinkerVaListClass = vmFuncs->internalFindClassUTF8(_currentThread, J9UTF8_DATA(utfWrapper), J9UTF8_LENGTH(utfWrapper), _vm->systemClassLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
		}
		omrthread_monitor_exit(_vm->exclusiveAccessMutex);
		...
		} else if ((type == _vm->memoryAddressClass)
		|| (type == _vm->cLinkerVaListClass)
		|| (J9CLASS_IS_ARRAY(type) && ((J9ArrayClass*)type)->leafComponentType == J9VMJAVALANGOBJECT_OR_NULL(_vm))
		) {
