...
ClassFileWriter::writeAttributes()
{
	...
	|| J9_ARE_ANY_BITS_SET(_romClass->extraModifiers, J9AccClassInnerClass)
		...
		for (UDATA i = 0; i < _romClass->enclosedInnerClassCount; i++, enclosedInnerClasses++) {
			...
			/* NOTE: outerClassInfoIndex (0 or non-zero), innerNameIndex and innerClassAccessFlags are not preserved
			 * in the ROM class - technically incorrect, but this should only matter to compilers.
			 */
