...
j9sysinfo_get_cache_info(struct J9PortLibrary *portLibrary, const J9CacheInfoQuery * query)
{
	 ...
	 * on Linux booted via QEMU (the emulator). In addition, it is still unclear
	 * whether all RISC-V CPUs support the cache line in this way. If no cache info
	 * is available on Linux, we should not assume it is literally supported on the hardware.
	 * In such case, there is no padding adjustment for class objects
	 * even if the -XX:-RestrictContended option is specified on the command line.
	 */
#if defined(RISCV64)
