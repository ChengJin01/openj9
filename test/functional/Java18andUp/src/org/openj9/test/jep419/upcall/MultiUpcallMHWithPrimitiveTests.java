...
public class MultiUpcallMHWithPrimitiveTests {
	...
	public void test_addLongAndLongFromPointerByUpcallMH_SameScope() throws Throwable {
		...
		try (ResourceScope scope = ResourceScope.newConfinedScope()) {
			...
			MemorySegment longSegmt1 = allocator.allocate(JAVA_LONG, 57424L);
			long result = (long)mh.invoke(longSegmt1, 698235L, upcallFuncAddr1);
			...
			NativeSymbol upcallFuncAddr2 = clinker.upcallStub(UpcallMethodHandles.MH_addLongAndLongFromPointer,
					FunctionDescriptor.of(JAVA_LONG, ADDRESS, JAVA_LONG), scope);
			MemorySegment longSegmt2 = allocator.allocate(JAVA_LONG, 57424L);
			result = (long)mh.invoke(longSegmt2, 698235L, upcallFuncAddr2);
			Assert.assertEquals(result, 755659L);

			NativeSymbol upcallFuncAddr3 = clinker.upcallStub(UpcallMethodHandles.MH_addLongAndLongFromPointer,
					FunctionDescriptor.of(JAVA_LONG, ADDRESS, JAVA_LONG), scope);
			MemorySegment longSegmt3 = allocator.allocate(JAVA_LONG, 57424L);
			result = (long)mh.invoke(longSegmt3, 698235L, upcallFuncAddr3);
			Assert.assertEquals(result, 755659L);
		}
	}
