...
public class InternalDowncallHandler {
	 ...
	 * MH.linkToNative().
	 */
	private native void resolveBoundHandle();
	/*[ENDIF] JAVA_SPEC_VERSION >= 22 */
	...
	private MethodHandle getNativeMHWithInvokeCache(MethodHandle handle) {
		nativeMH = SharedSecrets.getJavaLangInvokeAccess().nativeMethodHandle(handle);
		/* The cache array declared in the NativeMethodHandle (OpenJDK) stores MemberName
		 * and appendix resolved from the bounded handle.
		 */
		resolveBoundHandle();
		return nativeMH;
	}
