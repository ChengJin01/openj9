...
public class InternalDowncallHandler {
	...
	/* The hash map of native function address is intended for multithreading in which case
	 * the same downcall handler might hold various native function addresses being used by
	 * the different threads in downcall.
	 */
	/*[IF JAVA_SPEC_VERSION == 18]*/
	private ConcurrentHashMap<Thread, NativeSymbol> functAddrMap;
	/*[ELSE] JAVA_SPEC_VERSION == 18 */
	private ConcurrentHashMap<Thread, Addressable> functAddrMap;
	/*[ENDIF] JAVA_SPEC_VERSION == 18 */
	/*[IF JAVA_SPEC_VERSION >= 18]*/
	...
	private final long memAddrToLongArg(Addressable argValue) throws IllegalStateException {
		...
		/*[IF JAVA_SPEC_VERSION == 18]*/
			...
			if (!((NativeSymbol)argValue).scope().isAlive()) {
				throw new IllegalStateException("The scope of the native symbol is closed");  //$NON-NLS-1$
			}
	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		 ...
		 * since JDK17 so as to being passed in as the first argument when invoking the returned downcall
		 * handle or bound as the first argument via MethodHandles.insertArguments() beforehand in OpenJDK.
		 ...
		 */
		functAddrMap = new ConcurrentHashMap<>();
		/*[IF JAVA_SPEC_VERSION < 17]*/
		functAddrMap.put(Thread.currentThread(), downcallAddr);
		/*[ENDIF] JAVA_SPEC_VERSION < 17 */
	...
	private void SetDependency(MemorySession session) {
		...
		for (MemorySession memArgSession : sessionOrScopeSet) {
			...
			if (memArgSession.isAlive()) {
				...
				if ((owner == null) // For the shared session
				|| (owner == Thread.currentThread()) // For the confined session
				) {
	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		 ...
		 *
		 * Note: the passed-in downcalll symbol is always used for the current thread
		 * rather than the stored symbol in the hashmap which might become invalid given
		 * there might be multiple downcall symbols created in applications.
		 */
		functAddrMap.put(Thread.currentThread(), downcallAddr);
		functAddr = downcallAddr.address().toRawLongValue();
		/*[ELSE] JAVA_SPEC_VERSION >= 17 */
		functAddr = functAddrMap.get(Thread.currentThread());
		/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
		...
		if (!sessionOrScopeSet.isEmpty()) {
			...
			try (MemorySession nativeSession = MemorySession.openConfined()) {
				...
				returnVal = invokeNative(retMemAddr, functAddr, cifNativeThunkAddr, args);
			}
			/*[ELSE] JAVA_SPEC_VERSION >= 19 */
			/*[IF JAVA_SPEC_VERSION == 18]*/
