...
public class InternalDowncallHandler {
	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		...
		final long[] returnVal = new long[1];
		...
		if (sessionOrScope != null) {
			/*[IF JAVA_SPEC_VERSION >= 19]*/
			Thread invokeNativeThrd = new Thread(new Runnable(){
					@Override
					public void run(){
						returnVal[0] = invokeNative(retMemAddr, functionAddr.address().toRawLongValue(), cifNativeThunkAddr, args);
					}
				}
			);
			sessionOrScope.whileAlive(invokeNativeThrd);
			/*[ELSE] JAVA_SPEC_VERSION >= 19 */
			/*[IF JAVA_SPEC_VERSION == 18]*/
		...
		return Long.valueOf(returnVal[0]);
	}
}
