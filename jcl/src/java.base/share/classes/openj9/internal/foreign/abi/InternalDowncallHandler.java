	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		...
		if (!sessionOrScopeSet.isEmpty()) {
			...
			try (MemorySession nativeSession = MemorySession.openConfined()) {
				System.out.println("runNativeMethod: sessionOrScopeSet is NOT empty: call invokeNative: sessionOrScopeSet.size() = " + sessionOrScopeSet.size());
		...
		/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
		{
			System.out.println("runNativeMethod: sessionOrScopeSet is empty: call invokeNative: sessionOrScopeSet.size() = " + sessionOrScopeSet.size());
			returnVal = invokeNative(retMemAddr, functAddr, cifNativeThunkAddr, args);
		}
