...
public class InternalDowncallHandler {
	...
	/*[IF JAVA_SPEC_VERSION >= 19]*/
	private MemorySession sessionOrScope;
	/*[ELSE] JAVA_SPEC_VERSION >= 19 */
	private ResourceScope sessionOrScope;
	/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
	...
	/* Save the active session of the passed-in memory specific arguments in the downcall handler */
	/*[IF JAVA_SPEC_VERSION >= 19]*/
	private final MemorySession getActiveMemArgSession(MemorySession curSession, MemorySession memArgSession) {
		return (curSession == memArgSession) ? curSession : memArgSession;
	}
	/*[ELSE] JAVA_SPEC_VERSION >= 19 */
	private final ResourceScope getActiveMemArgScope(ResourceScope curScope, ResourceScope memArgScope) {
		return (curScope == memArgScope) ? curScope : memArgScope;
	}
	/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
	...
	private final long memAddrToLongArg(Addressable argValue) throws IllegalStateException {
		/* Only check MemorySegment and VaList given MemoryAddress.scope() doesn't exist in since JDK17 */
		if (argValue instanceof MemorySegment) {
			...
			sessionOrScope = getActiveMemArgSession(sessionOrScope, ((MemorySegment)argValue).session());
			System.out.println("memAddrToLongArg:JDK19: sessionOrScope = " + sessionOrScope);
			/*[ELSE] JAVA_SPEC_VERSION >= 19 */
			...
			sessionOrScope = getActiveMemArgScope(sessionOrScope ,((MemorySegment)argValue).scope());
			System.out.println("memAddrToLongArg:JDK18: sessionOrScope = " + sessionOrScope);
			/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
		} else if (argValue instanceof VaList) {
			/*[IF JAVA_SPEC_VERSION >= 19]*/
			if (!((VaList)argValue).session().isAlive()) {
				throw new IllegalStateException("The session of the VaList is closed");  //$NON-NLS-1$
			}
			sessionOrScope = getActiveMemArgSession(sessionOrScope, ((VaList)argValue).session());
			/*[ELSE] JAVA_SPEC_VERSION >= 19 */
			if (!((VaList)argValue).scope().isAlive()) {
				throw new IllegalStateException("The scope of the VaList is closed");  //$NON-NLS-1$
			}
			sessionOrScope = getActiveMemArgScope(sessionOrScope ,((VaList)argValue).scope());
			/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
		}
	...
	private final long memAddrToLongArg(MemoryAddress argValue) throws IllegalStateException {
		...
		sessionOrScope = getActiveMemArgScope(sessionOrScope ,argValue.scope());
		return argValue.address().toRawLongValue();
	}
	...
	private final long memSegmtToLongArg(MemorySegment argValue) throws IllegalStateException {
		...
		sessionOrScope = getActiveMemArgSession(sessionOrScope, argValue.session());
		/*[ELSE] JAVA_SPEC_VERSION >= 19 */
		/*[IF JAVA_SPEC_VERSION >= 17]*/
		if (!argValue.scope().isAlive())
		/*[ELSE] JAVA_SPEC_VERSION >= 17 */
	...
	/*[IF JAVA_SPEC_VERSION >= 19]*/
	/* Set up the dependency from the sub-session to the super session so as to
	 * keep the sub-session alive till the super session is closed.
	 */
	private void keepAlive(MemorySession supSession, MemorySession subSession) {
		Objects.requireNonNull(supSession);
		Objects.requireNonNull(subSession);
		MemorySessionImpl subSessionImpl = (MemorySessionImpl)subSession;
		subSessionImpl.acquire0();
		System.out.println("keepAlive: acquire0(): subSession = " + subSession);
		supSession.addCloseAction(subSessionImpl::release0);
	}
	/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		...
		long returnVal = 0;
		System.out.println("runNativeMethod: sessionOrScope = " + sessionOrScope);
		/* The session/scope associated with memory specific arguments must be kept alive in downcall since JDK18 */
		if (sessionOrScope != null) {
			/*[IF JAVA_SPEC_VERSION >= 19]*/
			try (MemorySession nativeSession = MemorySession.openConfined()) {
				keepAlive(nativeSession, sessionOrScope);
			...
			try (ResourceScope nativeScope = ResourceScope.newConfinedScope()) {
				nativeScope.keepAlive(sessionOrScope);
				returnVal = invokeNative(retMemAddr, functionAddr.address().toRawLongValue(), cifNativeThunkAddr, args);
			}
			/*[ELSE] JAVA_SPEC_VERSION == 18 */
			Handle scopeHandle = sessionOrScope.acquire();
			try {
				returnVal = invokeNative(retMemAddr, functionAddr.address().toRawLongValue(), cifNativeThunkAddr, args);
			} finally {
				sessionOrScope.release(scopeHandle);
			}
			/*[ENDIF] JAVA_SPEC_VERSION == 18 */
			/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
		} else {
