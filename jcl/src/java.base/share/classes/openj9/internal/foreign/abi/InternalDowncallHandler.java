...
public class InternalDowncallHandler {
	...
	/*[IF JAVA_SPEC_VERSION >= 19]*/
	private native long invokeNative(long returnStructMemAddr, long functionAddress, long calloutThunk, long[] argValues, MemorySession session);
	/*[ELSE] JAVA_SPEC_VERSION >= 19 */
	private native long invokeNative(long returnStructMemAddr, long functionAddress, long calloutThunk, long[] argValues, ResourceScope scope);
	/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
	...
	private final long memAddrToLongArg(Addressable argValue) throws IllegalStateException {
		...
		if (argValue instanceof MemorySegment) {
			...
			session = (session == null) ? ((MemorySegment)argValue).session() : session;
			/*[ELSE] JAVA_SPEC_VERSION >= 19 */
			...
			scope = (scope == null) ? ((MemorySegment)argValue).scope() : scope;
			/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
			...
			session = (session == null) ? ((VaList)argValue).session() : session;
			/*[ELSE] JAVA_SPEC_VERSION >= 19 */
			if (!((VaList)argValue).scope().isAlive()) {
				throw new IllegalStateException("The scope of the memory segment is closed");  //$NON-NLS-1$
			}
			scope = (scope == null) ? ((VaList)argValue).scope() : scope;
			/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
		}
	...
	private final long memAddrToLongArg(MemoryAddress argValue) throws IllegalStateException {
		...
		scope = (scope == null) ? argValue.scope() : scope;
		return argValue.address().toRawLongValue();
	}
	...
	private final long memSegmtToLongArg(MemorySegment argValue) throws IllegalStateException {
		...
		session = (session == null) ? argValue.session() : session;
		/*[ELSE] JAVA_SPEC_VERSION >= 19 */
		/*[IF JAVA_SPEC_VERSION >= 17]*/
		if (!argValue.scope().isAlive())
		/*[ELSE] JAVA_SPEC_VERSION >= 17 */
	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		...
		/*[IF JAVA_SPEC_VERSION >= 19]*/
		long returnVal = invokeNative(retMemAddr, functionAddr.address().toRawLongValue(), cifNativeThunkAddr, args, session);
		/*[ELSE] JAVA_SPEC_VERSION >= 19 */
		long returnVal = invokeNative(retMemAddr, functionAddr.address().toRawLongValue(), cifNativeThunkAddr, args, scope);
		/*[ENDIF] JAVA_SPEC_VERSION >= 19 */
