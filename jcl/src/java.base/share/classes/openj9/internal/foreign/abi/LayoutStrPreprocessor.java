...
final class LayoutStrPreprocessor {
	...
	private static StringBuilder composeStructLayoutStr(StringBuilder targetLayoutStr, int elemCount, int elemLayoutStrsLength) {
		 ...
		 * of the element layout string in computing the length of the struct layout string which is intended to
		 * hash the layout string in native in that
		 * 1) we need to compute the length of layout string to determine the boundary of the specified layout string in native
		 * when caching/searching the layout string hash (for struct or nested struct).
		 * 2) it's easy and straightforward to move the next layout (element) within the struct by skipping over the identified
		 * layout string (nested struct) with the computed layout length if its hash is found or cached in the hashtable.
		 *
