...
final class LayoutStrPreprocessor {
	...
	private static StringBuilder encodeUnionLayoutStr(UnionLayout unionLayout, StringBuilder targetLayoutStr, boolean isDownCall) {
		...
		/* The idea is to select the primitve type with the biggest alignement of the union
		 * (obtained via unionLayout.byteAlignment()) to be laid out as a sequence which is
		 * correctly mappped onto the given GPRs/FPRs on the hardware.
		 */
		...
		case 2: /* 2 Bytes. */
			...
			break;
		case 4: /* 4 Bytes. */
			if (validatePrimTypesOfUnionForGPR(unionLayout, byteAlignment)) {
	 ...
	 * non-floating-point primitves so as to determine whether the elements of union can be safely
	 * mapped to GPRs.
	 */
	private static boolean validatePrimTypesOfUnionForGPR(GroupLayout groupLayout, int byteAlignment) {
		List<MemoryLayout> elementLayoutList = groupLayout.memberLayouts();
		boolean arePrimTypesForGPR = false;
		for (MemoryLayout elementLayout : elementLayoutList) {
			if (((elementLayout instanceof ValueLayout elemValueLayout) && (validatePrimTypeForGPR(elemValueLayout, byteAlignment)))
				|| ((elementLayout instanceof GroupLayout elemGroupLayout) && validatePrimTypesOfUnionForGPR(elemGroupLayout, byteAlignment))
			) {
				arePrimTypesForGPR = true;
				break;
			}
		}

		return arePrimTypesForGPR;
	}

	/* Check whether the primitive type can be safely mappped to GPR when it is not float/double
	 * in the struct/union including some exceptions due to the platform restrictions.
	 *
	 * Note:
	 * The primitive type is mappped to GPR as long as it is not float/double on most platforms
	 ...
	 * 2) it is always mapped onto GPR in any case on zLinux.
	 */
	private static boolean validatePrimTypeForGPR(ValueLayout primLayout, int byteAlignment) {
		Class<?> carrier = primLayout.carrier();
		boolean isPrimTypeForGPR;

		if (carrier == float.class) {
			isPrimTypeForGPR = isLinuxS390x || ((8 == byteAlignment) && !isX86_64);
		} else if (carrier == double.class) {
			isPrimTypeForGPR = isLinuxS390x;
		} else {
			isPrimTypeForGPR = true;
		}

		return isPrimTypeForGPR;
	}
	/*[ENDIF] JAVA_SPEC_VERSION >= 21 */
