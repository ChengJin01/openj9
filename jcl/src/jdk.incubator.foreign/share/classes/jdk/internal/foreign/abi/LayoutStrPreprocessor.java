...
class LayoutStrPreprocessor {
	...
	static String getSimplifiedLayoutString(MemoryLayout targetLayout, boolean isDownCall) {
		String layoutString = preprocessLayout(targetLayout, isDownCall);
	 ...
	 */
	private static String preprocessLayout(MemoryLayout targetLayout, boolean isDownCall) {
		...
		if (targetLayout instanceof ValueLayout) {
			targetLayoutString = getPrimitiveTypeSymbol((ValueLayout)targetLayout);
			...
			targetLayoutString = elementCount + ":" + preprocessLayout(elementLayout, isDownCall); //$NON-NLS-1$
			...
			for (int elemIndex = 0; elemIndex < structElementCount; elemIndex++) {
				...
				if (structElement.isPadding()) {
					...
					elementLayoutStrs += preprocessLayout(structElement, isDownCall);
				}
			}
