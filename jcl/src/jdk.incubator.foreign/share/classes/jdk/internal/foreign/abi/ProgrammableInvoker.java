...
public class ProgrammableInvoker {
	...
	private void generateAdapter() {
		...
		synchronized(privateClassLock) {
			 ...
			 * 2) the same layout kind with or without layout name should be treated as the same layout.
			 * e.g.  C_INT without name = b32[abi/kind=INT]
			 *  and  C_INT with name = b32(int)[abi/kind=INT,layout/name=int]
			 */
	...
	private static String preprocessLayoutString(MemoryLayout targetLayout) {
		 ...
		 * e.g. remove "b32" for little-endianness and "B32" for big-endianness.
		 */
