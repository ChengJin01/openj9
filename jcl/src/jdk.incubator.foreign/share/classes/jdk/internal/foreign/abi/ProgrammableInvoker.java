...
public class ProgrammableInvoker {
	...
	private static final String osArch = System.getProperty("os.arch").toLowerCase(); //$NON-NLS-1$
	...
	private final MemorySegment longObjToMemSegmtRet(Object retValue) {
		long retMemAddr = ((Long)retValue).longValue();
		MemoryAddress retSegmtAddr = MemoryAddress.ofLong(retMemAddr);
		if (osArch.equals("aarch64")) { //$NON-NLS-1$
			/*[IF JAVA_SPEC_VERSION >= 17]*/
			return retSegmtAddr.asSegment(realReturnLayout.byteSize(), retSegmtAddr.scope());
			/*[ELSE] JAVA_SPEC_VERSION >= 17 */
			return retSegmtAddr.asSegmentRestricted(realReturnLayout.byteSize());
			/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
		} else {
	...
	/*[ENDIF] JAVA_SPEC_VERSION >= 17 */
	{
		...
		if ((funcMethodType.returnType() == MemorySegment.class)
		&& !osArch.equals("aarch64") //$NON-NLS-1$
		) {
			/*[IF JAVA_SPEC_VERSION >= 17]*/
