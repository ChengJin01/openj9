...
public class ProgrammableInvoker {
	...
	/* Preprocess the layout string to remove all descriptions or unnecessary setting for easier handling in native.
	...
	private static String preprocessLayoutString(MemoryLayout targetLayout) {
		...
		/* Remove all the primitives' size in the layout string (e.g. remove) "b32" for little-endianness and "B32" for big-endianness */
		resultLayoutString = resultLayoutString.replaceAll("b\\d+", "") //$NON-NLS-1$ //$NON-NLS-2$
					.replaceAll("B\\d+", "") //$NON-NLS-1$ //$NON-NLS-2$
					...
					.replaceAll("\\[abi/kind=LONG_LONG\\]", "J") //$NON-NLS-1$ //$NON-NLS-2$
					.replaceAll("\\[abi/kind=LONG\\]", "J") //$NON-NLS-1$ //$NON-NLS-2$
	...
	private static String prefixCountOfStructElements(MemoryLayout targetLayout) {
		...
		String resultLayoutString = targetLayout.toString().replaceAll("\\(.*?\\)", "") //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (GroupLayout.class.isInstance(resultLayout)) {
			...
			for (int elementIndex = 0; elementIndex < structElementCount; elementIndex++) {
				...
				/* Avoid parsing the padding element given it will be handled in native.
				 * Note: the padding bits should be equal to/less than 8 bytes (64 bits) in normal cases.
				 */
				if (structElement.isPadding()) {
					long paddingBits = structElement.bitSize();
					if (paddingBits > 64) {
						throw new IllegalArgumentException("The padding bits greater than 64 bits is not allowed: padding bits = " + paddingBits);  //$NON-NLS-1$
					}
					/* Only set up the tailing padding element of struct for the native function
					 * and ignore all padding elements in the middle given they are handled by
					 * ffi_call by default in native.
					 */
					if (elementIndex == (structElementCount - 1)) {
			...
			resultLayoutString = "#" + layoutElements.size() + "[" + elementLayoutStrs + "]"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			
			if (isSequenceLayout) {
				/* Prefix the count of sequence element to the front of the SequenceLayout string */
				resultLayoutString = seqElementCount + ":" + resultLayoutString; //$NON-NLS-1$
			}
