...
public class ProgrammableInvoker {
	...
	private static String preprocessLayoutString(MemoryLayout targetLayout) {
		...
		resultLayoutString = resultLayoutString.replaceAll("b\\d+", "") //$NON-NLS-1$ //$NON-NLS-2$
					...
					/* Remove all the padding bits in the layout string (e.g. remove "x16") given ffi_prep_cif() computes
					 * the alignment of arguments' ffi_type automatically regardless of the preset padding bits in the layout.
					 */
					.replaceAll("x\\d+", "") //$NON-NLS-1$ //$NON-NLS-2$
					/* Convert all primitives to a single character(symbol) for easier handling in native */
	...
	private static String prefixCountOfStructElements(MemoryLayout targetLayout) {
		...
		if (GroupLayout.class.isInstance(resultLayout)) {
			 ...
			 */
			resultLayoutString = "#" + (layoutElements.size() - paddingElementCount) //$NON-NLS-1$
