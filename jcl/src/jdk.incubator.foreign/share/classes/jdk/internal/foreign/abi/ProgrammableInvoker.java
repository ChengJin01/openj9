...
public class ProgrammableInvoker {
	...
	//private final MemorySegment funcMemSegReference;
	...
	private final Set<MemorySegment> memSegReferences = new HashSet<MemorySegment>();
	...
	ProgrammableInvoker(Addressable functionAddress, MethodType functionMethodType, FunctionDescriptor functionDescriptor) {
		MemoryAddress functionMemAddr = functionAddress.address();
		/* As explained in the Spec of LibraryLookup.Symbol, the intention of a confined memory segment
		 * being created here is to hold a strong reference to the native library lookup symbol set with
		 * the memory address of a underlying native function so as to prevent the corresponding native
		 * library from being unloaded automatically before use given the lifecycle of this segment
		 * depends on the external process.
		 */
		//funcMemSegReference = functionMemAddr.asSegmentRestricted(1, null, functionAddress);
		memSegReferences.add(functionMemAddr.asSegmentRestricted(1, null, functionAddress));
		functionAddr = functionMemAddr.toRawLongValue();
	...
	private void generateAdapter() {
		...
		synchronized(privateClassLock) {
			...
			System.out.println("*** generateAdapter: funcDescriptor = " + funcDescriptor  //$NON-NLS-1$
					...
					+ ", functionAddr = " + Long.toHexString(functionAddr)); //$NON-NLS-1$
	...
	public static MethodHandle getBoundMethodHandle(Addressable functionAddress, MethodType functionMethodType, FunctionDescriptor funcDesc) {
		checkIfInstanceOfLookupSymbol(functionAddress);
		checkIfValidType(functionMethodType, funcDesc.argumentLayouts().size());
		ProgrammableInvoker nativeInvoker = new ProgrammableInvoker(functionAddress, functionMethodType, funcDesc);
		try {
	...
	private static void checkIfInstanceOfLookupSymbol(Addressable functionAddress) {
		if (!Symbol.class.isInstance(functionAddress)) {
			throw new IllegalArgumentException("The passed-in argument " + functionAddress + " is not a lookup symbol of the specified native library");  //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
	...
	private static void checkIfValidType(MethodType targetMethodType, int argLayoutCount) {
		...
		for (int argIndex = 0; argIndex < argTypeCount; argIndex++) {
			if (!checkPrimitiveTypeClass(argTypes[argIndex]) && (argTypes[argIndex] != MemoryAddress.class)) {
	...
	private Object processReturnVal(long retValue) {
		...
		if (retLayoutClass == boolean.class) {
			...
			/* As explained in the Spec of MemoryAddress, the intention of a confined memory segment
			 * being created here is to hold a strong reference to the underlying memory address
			 * returned from the native code (e.g. custom allocators) so as to prevent it from being
			 * cleaned up/released automatically before use given the lifecycle of this segment
			 * depends on the external process.
			 */
			MemoryAddress memAddr = MemoryAddress.ofLong(retValue);
			//returnValueMemSegReference = memAddr.asSegmentRestricted(1, null, memAddr);
			memSegReferences.add(memAddr.asSegmentRestricted(1, null, memAddr));
			return memAddr;
		}
		return retValue;
	}
