...
public class ProgrammableInvoker {
	...
	public static MethodHandle getBoundMethodHandle(Addressable functionAddr, MethodType functionMethodType, FunctionDescriptor funcDesc) {
		...
		try {
			MethodHandle boundHandle = lookup.bind(nativeInvoker, "runNativeMethod", methodType(Object.class, Object[].class));
	...
	private static void checkIfValidType(MethodType targetMethodType) {
		...
		if (!checkPrimitiveTypeClass(retType) && (void.class != retType) && (MemoryAddress.class != retType)) {
			throw new InternalError("newType has non-primitive return type which is neither void nor MemoryAddress, only primtives, void and MemoryAddress are supported");  //$NON-NLS-1$
		}

		Class<?>[] argTypes = targetMethodType.parameterArray();
		for (int argIndex = 0; argIndex < argTypes.length; argIndex++) {
			if (!checkPrimitiveTypeClass(argTypes[argIndex]) && ( MemoryAddress.class != argTypes[argIndex])) {
				throw new InternalError("newType has non-primitive argument type which is not MemoryAddress, only primitives and MemoryAddress are supported"); //$NON-NLS-1$
			}
		}
	}
