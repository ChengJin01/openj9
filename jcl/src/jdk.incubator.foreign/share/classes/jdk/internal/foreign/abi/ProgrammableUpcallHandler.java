 ...
 */
/*[IF JAVA_SPEC_VERSION >= 18]*/
public class ProgrammableUpcallHandler { // UpcallHandler is removed in Java 18
...
/*[ENDIF] JAVA_SPEC_VERSION >= 18 */
	 ...
	 */
	ProgrammableUpcallHandler(MethodHandle target, MethodType mt, FunctionDescriptor cDesc) {
	...
	public static NativeSymbol makeUpcall(MethodHandle target, MethodType mt, FunctionDescriptor cDesc, ResourceScope scope) {
		ProgrammableUpcallHandler upcallHandler = new ProgrammableUpcallHandler(target, mt, cDesc);
	 ...
	 */
	public static UpcallHandler makeUpcall(MethodHandle target, MethodType mt, FunctionDescriptor cDesc) {
	...
	private long getUpcallThunkAddr(MethodHandle target) {
		 ...
		 */
		metaData = new UpcallMHMetaData();
		long addr = allocateUpcallStub(target, metaData, nativeSignatureStrs);
		return addr;
	}
	 ...
	 */
	private native long allocateUpcallStub(MethodHandle calleeMH, UpcallMHMetaData mhMetaData, String[] cSignatureStrs);
