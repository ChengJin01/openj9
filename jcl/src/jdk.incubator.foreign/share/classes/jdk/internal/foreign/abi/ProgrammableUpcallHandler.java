	...
	private long getUpcallThunkAddr(MethodHandle target, ResourceScope scope) {
		...
		synchronized(privateUpcallClassLock) {
			...
			if (upcallThunkAddr != null) {
				...
				System.out.println("getUpcallThunkAddr: SAME SCOPE: " + targetHashScopeStr + ", addr: " + addr); //$NON-NLS-1$ //$NON-NLS-2$
			} else {
				...
				addr = allocateUpcallStub(target, metaData, nativeSignatureStrs);
				cachedUpcallThunkAddr.put(upcallThunkAddrKey, Long.valueOf(addr));
				System.out.println("getUpcallThunkAddr: DIFF SCOPE: " + targetHashScopeStr + ", addr: " + addr); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
